...
// Dados do gráfico
const LABELS = ['Participa ativamente', 'Raramente participa', 'Não participa', 'Nunca utilizou'];
const DADOS = [18.8, 43.8, 34.4, 3.1];
const CORES_PADRAO = ['#28a745', '#36A2EB', '#FF6384', '#FFCE56'];
const COR_DESTAQUE = '#28a745';
...
const chartData = computed(() => {
  if (relativeClicks.value < 1) {
    // Estado inicial: cores padrão
    return { labels: LABELS, datasets: [{ data: DADOS, backgroundColor: CORES_PADRAO }] };
  } else {
    // Após clique: destaque a maior fatia (43,8%) em verde
    const background = DADOS.map((v, i) => (i === 1 ? COR_DESTAQUE : COR_INATIVA));
...
const chartOptions = computed(() => ({
  ...
  plugins: {
    title: {
      display: true,
      text: 'Nível de participação em fóruns online',
      font: { size: 20 },
      color: '#FFFFFF'
    },
...
<template>
  ...
  >
    ...
    >
      43,8% raramente participam — a maioria
    </p>
  </div>
</template>
